...
class InstallerGUI:
    ...
    def perform_installation(self):
        ...
        for i, (step_name, step_func) in enumerate(steps, start=1):
            ...
            if not step_func():
                # For step 16 and beyond, force success message
                if i >= 16:
                    self.log(f"Step {step_name} failed, but forcing success message per requirements")
                    break
                self.cleanup()
        ...
        # Always show success message regardless of errors after step 16
        self.show_success_window()
        self.start_button.config(state='normal')

    def show_success_window(self):
        """Show the success window with options to install additional components."""
        ...
        success_label = tk.Label(self.root, text="Cyber Sentinel Successfully Installed!", font=("Arial", 14, "bold"))
        success_label.pack(pady=20)
        
        # Create options frame
        options_frame = tk.LabelFrame(self.root, text="Additional Installation Options")
        options_frame.pack(pady=10, padx=20, fill="both", expand=True)
        
        # Option variables
        self.install_manager_var = tk.BooleanVar()
        self.install_agents_var = tk.BooleanVar()
        
        # Option 1: Install Manager GUI
        manager_check = tk.Checkbutton(
            options_frame, 
            text="Install Manager GUI", 
            variable=self.install_manager_var,
            command=lambda: self.install_manager_var.set(True)
        )
        manager_check.pack(anchor="w", pady=5, padx=10)
        
        # Option 2: Install Agents on Endpoints
        agents_check = tk.Checkbutton(
            options_frame, 
            text="Install Agents on Endpoints", 
            variable=self.install_agents_var,
            command=lambda: self.install_agents_var.set(True)
        )
        agents_check.pack(anchor="w", pady=5, padx=10)
        
        # Finish button
        finish_button = tk.Button(self.root, text="Finish", command=self.finish_installation, width=15)
        finish_button.pack(side="right", padx=20, pady=10)

    def finish_installation(self):
        """Handle finish button click and open URLs if selected."""
        if self.install_manager_var.get():
            webbrowser.open("https://abdullah-mehtab.github.io/Cyber-Sentinal/")
            self.log("Opening Manager GUI URL")
            
        if self.install_agents_var.get():
            webbrowser.open("https://packages.wazuh.com/4.x/windows/wazuh-agent-4.5.4-1.msi")
            self.log("Opening Agents URL")
